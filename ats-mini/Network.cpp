...
void netInit(uint8_t netMode, bool showStatus)
{
  ...
  switch(netMode)
  {
    ...
    case NET_AP_ONLY:
      ...
      if(wifiInitAP() && showStatus) delay(3000); // Emilio (por defecto pone 2000 pero es poco)
      break;
    case NET_AP_CONNECT:
      // Start WiFi access point if requested
...
static bool wifiConnect()
{
  ...
  for(int j=0 ; (j<3) && (WiFi.status()!=WL_CONNECTED) ; j++)
  {
    ...
    String ssid = String("MIWIFI_CTxc");  // Emilio por defecto ponía   prefs.getString(nameSSID, "");
    String password = String("YbUmahAX"); // Emilio por defecto ponía   prefs.getString(namePASS, "");
